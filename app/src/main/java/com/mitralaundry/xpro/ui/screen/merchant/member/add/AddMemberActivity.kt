...
class AddMemberActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val user = intent?.extras?.get("name")
        val saldo = intent?.extras?.get("saldo")
        val phoneNumber = intent?.extras?.get("phone_number")
        val outletId = intent?.extras?.get("outlet_id")
        val merchantId = intent?.extras?.get("merchant_id")
        val memberId = intent?.extras?.get("member_id")

        viewModel.setUserName(user.toString())
        viewModel.setSaldo(saldo.toString().filter { it.isDigit() }.toInt())
        viewModel.setPhoneNumber(phoneNumber.toString())
        viewModel.setOutletId(outletId.toString())
        viewModel.setMerchantId(merchantId.toString())
        viewModel.setMemberId(memberId.toString())
